    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (item[i].Hodnota > 0)
                {
                    Aktiv = $"| {item[i].Nazev,-16}| {item[i].Typ,-16} | {Prevod(item[i].Hodnota, item[i].Typ),-15:C} |";
                    if (i + 1 != 4)
                    {
                        if (item[i + 1].Hodnota < 0)
                        {
                            Pasiv = $" {item[i + 1].Nazev,-16} | {item[i + 1].Typ,-16} | {Prevod(item[i + 1].Hodnota, item[i + 1].Typ),-15:C} |";
                            i++;
                        }
                    }
                }
                ...
                {
                    Pasiv = $" {item[i].Nazev,-16} | {item[i].Typ,-16} | {Prevod(item[i].Hodnota, item[i].Typ),-15:C} |";
                    if (i + 1 != 4)
                    {
                        if (item[i + 1].Hodnota > 0)
                        {
                            Aktiv = $"| {item[i + 1].Nazev,-16}| {item[i + 1].Typ,-16} | {Prevod(item[i + 1].Hodnota, item[i].Typ),-15:C} |";
                            i++;
                        }
                    }
                }
